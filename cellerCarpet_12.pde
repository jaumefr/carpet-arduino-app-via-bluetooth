...
void compara(){
      println("sensor1: "+sensor1+"   substring: "+serie.substring(min,max));
...
void mouseClicked(){ // --------------------- CONTROL MANUAL
        for(int m=0; m<5; m++){
          //println(m+": "+tags[m].manual());
          if(tags[m].manual()){ 
              sensor1 = str(m+1); 
              break; 
          }
        }
        //println("mouse: "+sensor1);
        compara();
}